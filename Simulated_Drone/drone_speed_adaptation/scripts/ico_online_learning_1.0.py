...
class MavController:
    ...
    def twist_callback(self, data):
        """
        Handle local twist information
        """
        #self.timestamp = data.header.stamp
...
def ICO_Learning():
    ...
    global ICO_activated, ICO_output, ICO_learning_rate, distance, count
    ...
    predictive_signal = (-1.0/2.5)*distance + 1.20        # mapping range from 0.5 ~ 3.0 to 1.0 ~ 0.0
    #if predictive_signal > 1.0:
    #predictive_signal = 1.0
...
def simple_demo():
    ...
    """
    global px, py, pz, px1, py1, pz1, ICO_output, count
    f = open(r"/home/vatsanaij/catkin_ws/src/drone_speed_adaptation/scripts/Result_Record/mavros_adapt_speed_online.txt","w")
    c = MavController()
    rospy.sleep(1)
    ...
    while t_iteration <=  10 :
        ...
        while loop <= 10:
            ...
            if key == 'i' :
                ...
                c.goto_xyz_rpy(-4.5,0,1.5,0,0,0)
                rospy.sleep(3)
            if key == 's' :
                print ("Start testing")
        ...
        for i in range (1, 170):
            ...
            print("px: %f\t py: %f\t pz: %f\t vx: %f\t vy: %f\t vz: %f\t roll: %f\t pitch: %f\t yaw: %f\t DT: %f\t p_signal: %f\t r_signal: %f\t distance: %f\t ICO_out: %f\t vel_x: %f\t p_weight: %f\n" % (px,py,pz,c.vx,c.vy,c.vz,roll,pitch,yaw,deltaT,predictive_signal,reflexive_signal,distance,ICO_output,vel_x,predictive_W))
            f.write("%f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %f\t %d\n" % (px,py,pz,c.vx,c.vy,c.vz,roll,pitch,yaw,deltaT,predictive_signal,reflexive_signal,distance,ICO_output,vel_x,predictive_W,t_iteration))
            ...
            elif ICO_output > 0.0 and ICO_output < 0.9 :
